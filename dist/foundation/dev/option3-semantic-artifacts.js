...
export function createOption3SemanticOverlaySvg(referenceHref = REPOSITORY_REFERENCE_HREF) {
    ...
    const facePolygons = renderModel.faces.map((face) => {
        ...
        if (!row)
            throw new Error(`Option-3 diagnostic has no semantic binding for ${face.id}.`);
    ...
    const wallBands = renderModel.walls.map((wall) => `    <line class="wall-band" data-wall-id="${wall.id}" x1="${wall.x1}" y1="${wall.y1}" x2="${wall.x2}" y2="${wall.y2}" stroke-width="${wall.strokeWidth}" />`);
    const centreLines = renderModel.walls.map((wall) => `    <line class="wall-centre" data-wall-id="${wall.id}" x1="${wall.x1}" y1="${wall.y1}" x2="${wall.x2}" y2="${wall.y2}" />`);
    const labels = rows.map((row) => {
        const face = renderModel.faces.find((candidate) => candidate.id === row.faceId);
        if (!face)
            throw new Error(`Option-3 diagnostic cannot place semantic label for ${row.faceId}.`);
...
export function createOption3SemanticMappingMarkdown() {
    const rows = createOption3SemanticSpaceMappingRows();
    return [
        ...
        "|---|---|---|---|---:|---|",
        ...rows.map((row) => `| \`${row.id}\` | ${escapeMarkdownCell(row.name)} | \`${row.category}\` | \`${row.faceId}\` | ${row.areaLabel} | ${escapeMarkdownCell(row.locationEvidence)} Boundary (${row.boundaryWallIds.length} directed references): ${row.boundaryWallIds.map((id) => `\`${id}\``).join(", ")} |`),
        "",
...
//# sourceMappingURL=option3-semantic-artifacts.js.map