...
import { wallId, } from "../topology/model.js";
 ...
 */
export function extractBoundedFaces(topologyValue) {
...
function findBridgeWallIds(topology) {
    ...
    for (const edges of adjacency.values())
        edges.sort((left, right) => left.wallId.localeCompare(right.wallId));
    const discovery = new Map();
    const low = new Map();
    const bridges = new Set();
    let time = 0;
    const visit = (nodeIdentity, parentWallId) => {
        ...
        for (const edge of adjacency.get(nodeIdentity)) {
            ...
            if (neighbourDiscovery === undefined) {
                ...
                if (low.get(edge.nodeId) > discoveredAt)
                    bridges.add(edge.wallId);
            }
            else {
    ...
    for (const nodeIdValue of Object.keys(topology.nodes).sort().map((value) => value)) {
        if (!discovery.has(nodeIdValue))
            visit(nodeIdValue, null);
    }
    return bridges;
}
function createHalfEdges(topology, bridges) {
    const result = new Map();
    for (const wallIdValue of Object.keys(topology.walls).sort().map(wallId)) {
        if (bridges.has(wallIdValue))
            continue;
...
function directionRank(topology, edge) {
    const start = topology.nodes[edge.startNodeId];
    const end = topology.nodes[edge.endNodeId];
    if (end.xUm > start.xUm)
        return 0; // east
    if (end.yUm > start.yUm)
        return 1; // south in the ProjectV2/SVG coordinate frame
    if (end.xUm < start.xUm)
        return 2; // west
    return 3; // north
}
function createSuccessorMap(outgoing, halfEdges) {
    const result = new Map();
    for (const edge of halfEdges.values()) {
        const atDestination = outgoing.get(edge.endNodeId);
        if (!atDestination?.length)
            throw new FaceExtractionError(`No outgoing half-edge at node ${edge.endNodeId}.`);
        const incomingTwinIndex = atDestination.findIndex((candidate) => candidate.key === twinKey(edge));
        if (incomingTwinIndex < 0)
            throw new FaceExtractionError(`Missing twin half-edge for ${edge.key}.`);
...
function traceWalk(start, halfEdges, successors, visited) {
    const walk = [];
    const local = new Set();
    let current = start;
    while (true) {
        if (local.has(current.key)) {
            if (current.key !== start.key)
                throw new FaceExtractionError(`Half-edge walk merged into a cycle at ${current.key}.`);
            break;
        }
        if (visited.has(current.key))
            throw new FaceExtractionError(`Half-edge ${current.key} belongs to multiple face walks.`);
        ...
        if (!next)
            throw new FaceExtractionError(`Half-edge ${current.key} has no valid successor.`);
        current = next;
        if (walk.length > halfEdges.size)
            throw new FaceExtractionError("Half-edge traversal did not terminate.");
    }
    return walk;
}
function calculateSignedDoubleArea(topology, walk) {
    let result = 0n;
...
function createFaceCandidate(topology, walk, signedDoubleArea) {
...
function rotateToCanonicalStart(walk) {
    let bestIndex = 0;
    for (let index = 1; index < walk.length; index += 1) {
        if (walk[index].key.localeCompare(walk[bestIndex].key) < 0)
            bestIndex = index;
    }
    return [...walk.slice(bestIndex), ...walk.slice(0, bestIndex)];
}
function assertSimpleBoundary(vertices, signature) {
    if (vertices.length < 4)
        throw new FaceExtractionError(`Bounded face ${signature} has fewer than four orthogonal edges.`);
    const seen = new Set();
    for (const vertex of vertices) {
        if (seen.has(vertex.nodeId))
            throw new FaceExtractionError(`Bounded face ${signature} repeats node ${vertex.nodeId}.`);
        seen.add(vertex.nodeId);
    }
}
function assertNoOverlappingFaceInteriors(faces) {
    for (let leftIndex = 0; leftIndex < faces.length; leftIndex += 1) {
        for (let rightIndex = leftIndex + 1; rightIndex < faces.length; rightIndex += 1) {
            ...
            if (pointStrictlyInsidePolygon(interiorSample(left.vertices), right.vertices) ||
                ...
                throw new FaceExtractionError(`Derived faces ${left.id} and ${right.id} have overlapping interiors; nested disconnected cycles require explicit hole support.`);
            }
        }
    }
}
function interiorSample(vertices) {
...
function pointStrictlyInsidePolygon(point, vertices) {
    let inside = false;
...
//# sourceMappingURL=extract-faces.js.map