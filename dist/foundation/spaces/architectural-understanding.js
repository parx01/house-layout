...
export function deriveArchitecturalUnderstanding(topologyValue, spacesValue) {
    ...
    const semanticSpaceIdsByWallId = new Map();
...
function indexFacesByWall(faces) {
    ...
    for (const faceIds of result.values())
        faceIds.sort();
    return result;
}
function deriveClearRegion(topology, face) {
    const faceBounds = boundsOfFace(face);
    const bands = Object.keys(topology.walls)
        .sort()
        .map((identity) => wallBand(topology, identity))
    ...
    const cells = [];
    const clearGrid = Array.from({ length: xs.length - 1 }, () => Array.from({ length: ys.length - 1 }, () => false));
    let quarterArea = 0n;
    ...
    if (!Number.isSafeInteger(areaNumerator))
        throw new RangeError(`Clear area for face ${face.id} exceeds safe precision.`);
...
function describeClearGeometry(region) {
    if (!region.bounds || region.cells.length === 0) {
        return { status: "noClearInterior", clearAreaUm2: 0, horizontalSegments: [], verticalSegments: [] };
    }
    const boundsAreaQuarter = BigInt(region.bounds.right2 - region.bounds.left2) * BigInt(region.bounds.bottom2 - region.bounds.top2);
...
function dimensionSegments(cells, orientation) {
    ...
    for (const values of grouped.values()) {
        const sorted = [...values].sort((left, right) => orientation === "horizontal" ? left.left2 - right.left2 : left.top2 - right.top2);
...
function verticalSegment(top2, bottom2, x2) {
    return {
...
function labelAnchor(face, region) {
...
function largestClearRectangle(xs, ys, grid) {
    let best = null;
        ...
        for (let right = left + 1; right < xs.length; right += 1) {
            for (let top = 0; top < ys.length - 1; top += 1) {
                for (let bottom = top + 1; bottom < ys.length; bottom += 1) {
                    if (!rectangleIsClear(prefix, left, right, top, bottom))
                        continue;
...
function rectangleIsClear(prefix, left, right, top, bottom) {
...
function clearCellPrefix(grid, width, height) {
...
function largestCentreLineCell(face) {
    ...
    for (let x = 0; x < xs.length - 1; x += 1) {
        for (let y = 0; y < ys.length - 1; y += 1) {
            ...
            if (!pointInsideFaceQuarterUnits(face, x4, y4))
                continue;
            const area = BigInt(xs[x + 1] - xs[x]) * BigInt(ys[y + 1] - ys[y]);
            const point = { xUm: x4 / 4, yUm: y4 / 4 };
            if (!best || area > best.area || (area === best.area && comparePoint(point, best.point) < 0))
                best = { point, area };
        }
    }
    if (!best)
        throw new Error(`Face ${face.id} has no deterministic interior label position.`);
    return best.point;
}
function comparePoint(left, right) {
    return left.yUm - right.yUm || left.xUm - right.xUm;
}
function boundsOfFace(face) {
    return {
...
function wallBand(topology, wallIdentity) {
...
function pointInsideBandQuarterUnits(bounds, x4, y4) {
    return x4 > bounds.left2 * 2 && x4 < bounds.right2 * 2 && y4 > bounds.top2 * 2 && y4 < bounds.bottom2 * 2;
}
function pointInsideFaceQuarterUnits(face, x4, y4) {
    let inside = false;
    for (let index = 0; index < face.vertices.length; index += 1) {
        ...
        if (y4 > minY4 && y4 < maxY4 && start.xUm * 4 > x4)
            inside = !inside;
    }
    return inside;
}
...
//# sourceMappingURL=architectural-understanding.js.map