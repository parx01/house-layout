...
Exact duplicate node pairs, including reversed pairs, are rejected. General partial collinear overlap detection is intentionally deferred to A2.2, alongside segment normalization. A committed T-junction or X-intersection must ultimately use an explicit shared node and split canonical segments; automatic intersection detection and splitting are also A2.2 responsibilities.
