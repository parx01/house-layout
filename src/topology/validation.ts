...
function validateCanonicalRelationships(
  ...
  for (let leftIndex = 0; leftIndex < wallValues.length; leftIndex += 1) {
    ...
    for (let rightIndex = leftIndex + 1; rightIndex < wallValues.length; rightIndex += 1) {
      ...
      const intersects =
        between(verticalStart.xUm, horizontalStart.xUm, horizontalEnd.xUm) &&
  ...
  for (const node of Object.values(nodes)) {
    for (const wall of wallValues) {
      if (wall.startNodeId === node.id || wall.endNodeId === node.id) continue;
      const start = nodes[wall.startNodeId]!;
      const end = nodes[wall.endNodeId]!;
...
function pointStrictlyInsideSegment(point: TopologyNode, start: TopologyNode, end: TopologyNode): boolean {
