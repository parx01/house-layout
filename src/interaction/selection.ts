...
export type CanonicalSelectableEntity =
  | { readonly type: "space"; readonly id: SpaceId }
...
export function createCanonicalSelectionState(): CanonicalSelectionState {
  return selectionState(null, null);
}

export function setHoveredEntity(
  state: CanonicalSelectionState,
  entity: CanonicalSelectableEntity | null,
): CanonicalSelectionState {
  return selectionState(entity, state.selected);
}

export function clearHoveredEntity(state: CanonicalSelectionState): CanonicalSelectionState {
  return setHoveredEntity(state, null);
}

export function selectCanonicalEntity(
  state: CanonicalSelectionState,
  entity: CanonicalSelectableEntity,
): CanonicalSelectionState {
  return selectionState(state.hovered, entity);
}

export function clearCanonicalSelection(state: CanonicalSelectionState): CanonicalSelectionState {
  return selectionState(state.hovered, null);
}

export function createCanonicalHitTestModel(
  topologyValue: TopologyV2,
  spacesValue?: SemanticSpacesV2,
): CanonicalHitTestModel {
...
export function hitTestCanonicalSelection(
  model: CanonicalHitTestModel,
  point: SelectionHitPoint,
  viewport: SelectionViewportScale,
): CanonicalHitTestResult | null {
  assertFinitePoint(point);
  assertViewport(viewport);

  const nodes = model.nodes
    .map((node) => ({
      entity: { type: "node" as const, id: node.id },
  ...
  const walls = model.walls
    .map((wall) => ({
      entity: { type: "wall" as const, id: wall.id },
      distancePx: distanceToSegmentPx(point, wall.start, wall.end, viewport),
      hitHalfWidthPx: Math.max(
        WALL_MINIMUM_HIT_HALF_WIDTH_PX,
        wall.thicknessUm / 2 / perpendicularScale(wall, viewport),
      ),
    }))
    .filter((candidate) => candidate.distancePx <= candidate.hitHalfWidthPx)
    .sort(compareHits);
  if (walls[0]) return { entity: walls[0].entity, distancePx: walls[0].distancePx };
...
export function reconcileCanonicalSelection(
  state: CanonicalSelectionState,
  projectValue: ProjectV2,
  remap: CanonicalSelectionRemap = {},
): CanonicalSelectionState {
  ...
  return selectionState(
    reconcileEntity(state.hovered, project, remap),
    reconcileEntity(state.selected, project, remap),
  );
}
...
export function resolveCanonicalSelection(
  projectValue: ProjectV2,
  entity: CanonicalSelectableEntity | null,
): ResolvedCanonicalSelection | null {
  if (!entity) return null;
  const project = validateProjectV2(projectValue);
  if (project.topology.status !== "active") return null;
  const topology = project.topology;
  if (entity.type === "space") {
    if (project.spaces.status !== "active") return null;
...
function selectionState(
  hovered: CanonicalSelectableEntity | null,
  selected: CanonicalSelectableEntity | null,
): CanonicalSelectionState {
  return Object.freeze({ hovered: freezeEntity(hovered), selected: freezeEntity(selected) });
}

function freezeEntity(entity: CanonicalSelectableEntity | null): CanonicalSelectableEntity | null {
...
function reconcileEntity(
  ...
  project: ProjectV2,
  remap: CanonicalSelectionRemap,
): CanonicalSelectableEntity | null {
  if (!entity) return null;
  if (entityExists(project, entity)) return entity;
  ...
  const mapped = { type: entity.type, id: mappedId } as CanonicalSelectableEntity;
  return entityExists(project, mapped) ? mapped : null;
}

function entityExists(project: ProjectV2, entity: CanonicalSelectableEntity): boolean {
  if (project.topology.status !== "active") return false;
  if (entity.type === "wall") return project.topology.walls[entity.id] !== undefined;
  if (entity.type === "node") return project.topology.nodes[entity.id] !== undefined;
  return project.spaces.status === "active" &&
    project.spaces.spaces.some((space) => space.id === entity.id);
}

function assertOneToOne(values: Readonly<Record<string, string>> | undefined, label: string): void {
  if (!values) return;
...
function assertFinitePoint(point: SelectionHitPoint): void {
...
function assertViewport(viewport: SelectionViewportScale): void {
  if (
    !Number.isFinite(viewport.xUmPerCssPixel) || viewport.xUmPerCssPixel <= 0 ||
...
function scaledDistance(
  point: SelectionHitPoint,
  target: SelectionHitPoint,
  viewport: SelectionViewportScale,
): number {
  return Math.hypot(
    (target.xUm - point.xUm) / viewport.xUmPerCssPixel,
    (target.yUm - point.yUm) / viewport.yUmPerCssPixel,
  );
}

function distanceToSegmentPx(
  point: SelectionHitPoint,
  start: SelectionHitPoint,
  end: SelectionHitPoint,
  viewport: SelectionViewportScale,
): number {
...
function perpendicularScale(wall: SelectionHitWall, viewport: SelectionViewportScale): number {
  return wall.orientation === "horizontal" ? viewport.yUmPerCssPixel : viewport.xUmPerCssPixel;
}

function compareHits(
  left: { readonly entity: CanonicalSelectableEntity; readonly distancePx: number },
  right: { readonly entity: CanonicalSelectableEntity; readonly distancePx: number },
): number {
  return left.distancePx - right.distancePx || left.entity.id.localeCompare(right.entity.id);
}

function pointInsideOrOnPolygon(point: SelectionHitPoint, vertices: readonly DerivedFaceVertex[]): boolean {
  let inside = false;
  for (let index = 0, previous = vertices.length - 1; index < vertices.length; previous = index++) {
    const start = vertices[previous]!;
    const end = vertices[index]!;
    if (pointOnSegment(point, start, end)) return true;
    if ((end.yUm > point.yUm) === (start.yUm > point.yUm)) continue;
    const intersectionX = start.xUm +
      ((point.yUm - start.yUm) * (end.xUm - start.xUm)) / (end.yUm - start.yUm);
    if (point.xUm < intersectionX) inside = !inside;
  }
  return inside;
}
