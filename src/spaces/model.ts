...
export type FaceId = string & { readonly [faceIdBrand]: "FaceId" };
...
export function faceId(value: string): FaceId {
  if (!/^f-[a-f0-9]{16}$/.test(value)) throw new TypeError(`Invalid derived face ID: ${JSON.stringify(value)}.`);
  return value as FaceId;
}
